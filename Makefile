...
# the cross-compiler AND LhA; the result (build/narrator-wyoming.lha) plus the
# repo's narrator-wyoming.readme are what you upload to Aminet.
dist:
	$(DOCKER_RUN) make dist-pack

# Stage the drop-in pair + docs + installer and pack them. The archive holds a
# single narrator-wyoming/ drawer. (Container-only: builds the Amiga binaries.)
dist-pack: $(DEV_BIN) $(LIB_BIN) narrator-wyoming.readme docs/README.txt dist/Install dist/Install.info LICENSE config/narrator.wyoming.example
	rm -rf $(BUILD)/dist $(DISTLHA)
	mkdir -p $(DISTSTAGE)
	cp $(DEV_BIN)                      $(DISTSTAGE)/narrator.device
	cp $(LIB_BIN)                      $(DISTSTAGE)/translator.library
	cp narrator-wyoming.readme         $(DISTSTAGE)/narrator-wyoming.readme
	sed -i.bak "s/^Version:.*/Version:      $(VERSION).$(REVISION)/" $(DISTSTAGE)/narrator-wyoming.readme && rm -f $(DISTSTAGE)/narrator-wyoming.readme.bak
	...
	@echo "Created $(DISTLHA) (upload it + narrator-wyoming.readme to Aminet)"
...
readme-version:
	@sed -i.bak "s/^Version:.*/Version:      $(VERSION).$(REVISION)/" narrator-wyoming.readme && rm -f narrator-wyoming.readme.bak
	@echo "narrator-wyoming.readme Version: $(VERSION).$(REVISION)"
...
release:
	@$(MAKE) --no-print-directory readme-version
	$(MAKE) clean dist
	@if ! git diff --quiet HEAD -- narrator-wyoming.readme; then \
	   git add narrator-wyoming.readme; \
